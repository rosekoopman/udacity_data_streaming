...
# write to kafka -- note: it takes up to a few minutes for the new topic to occur!
vehicle_status_checkin_stream_df\
    .selectExpr("cast(vehicle_status_vw.truckNumber as string) as key", "to_json(struct(*)) as value")\
    .writeStream\
    .format("kafka")\
    .option("kafka.bootstrap.servers","localhost:9092")\
    .option("topic","vehicle-status-checkin")\
