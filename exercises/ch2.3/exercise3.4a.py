...
# Note to myself: solution as presented in course
...
class ClickEvent:
    ...
    #            https://github.com/confluentinc/confluent-kafka-python/blob/master/src/confluent_kafka/avro/load.py
    #
    string_schema = """{
        "type": "record",
        "name": "click_event",
    ...
    schema = avro.loads(string_schema) 
...
async def produce(topic_name):
    ...
    #            https://github.com/confluentinc/confluent-kafka-python/blob/master/src/confluent_kafka/avro/cached_schema_registry_client.py#L46
    #
    
    # Note to myself: the wat CachedSchemaRegistyClient is initialized here (confluent_kafka version 1.1.0) is depricated in the latest version (1.7.0)
    #schema_registry = CachedSchemaRegistryClient(SCHEMA_REGISTRY_URL)   # as in course but depricated
    ...
    # avroproducer as in course
    p = AvroProducer({"bootstrap.servers": BROKER_URL},
                      schema_registry=schema_registry)
    # avroproducer as in docs (confluent kafka 1.7.0)
#     p = AvroProducer({"bootstrap.servers": BROKER_URL,
...
async def consume(topic_name):
    ...
    # avro consumer as presented in course
    c = AvroConsumer({"bootstrap.servers": BROKER_URL, 
                      "group.id": "0"},
                     schema_registry=schema_registry)
    # avro consumer as in docs (confluent kafka version 1.7.0)
#     c = AvroConsumer({"bootstrap.servers": BROKER_URL, 
#                       "group.id": "0",
#                       "schema.registry.url": SCHEMA_REGISTRY_URL})
