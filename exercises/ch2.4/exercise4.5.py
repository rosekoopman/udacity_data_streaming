...
def produce():
    ...
    #       See: https://docs.confluent.io/current/kafka-rest/api.html#post--topics-(string-topic_name)
    resp = requests.post(
        f"{REST_PROXY_URL}/topics/ex4.5", data=json.dumps(data), headers=headers  # TODO
    )
...
# kafka-console-consumer --topic ex4.5 --bootstrap-server localhost:9092

# note: if a key_schema is defined, also a value for the key need to be added to
# the record in line 27