...
async def consume(topic_name):
    ...
    n = 0
    while True:
        messages = c.consume(100, timeout=0.1)
        
        for i, message in enumerate(messages):
            if message is None:
                print(f'{i} {n} No message received')
            elif message.error() is not None:
                print(f'{i} {n} Received an error: {message.error()}')
            else:
                print(f'{i} {n} Message: Key = {message.key()} \t Value = {message.value()}')
            n += 1
        #
